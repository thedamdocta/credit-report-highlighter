...
export const CreditReportAnalyzerApp = () => {
  ...
  const handleDownloadProcessedPDF = useCallback(async () => {
    ...
    try {
      // Mock download functionality - replace with your actual HighlightGenerator
      console.log('Downloading PDF with highlights...');
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Create a mock download
      const blob = new Blob(['Mock highlighted PDF content'], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `highlighted_${uploadedFile.name}`;
        ...
        {uploadedFile && (
          <div className="absolute bottom-0 left-0 right-0 bg-transparent">
            ...
            {analysisResult?.highlights && analysisResult.highlights.length > 0 && (
              <div className="px-4 py-3 border-b border-gray-200 bg-white/90 backdrop-blur-sm">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className="bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors"
                  >
                    {isDownloading ? (
                      <>
